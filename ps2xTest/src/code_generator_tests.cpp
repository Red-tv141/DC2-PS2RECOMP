...
void register_code_generator_tests()
{
    ...
    tc.Run("jump to unknown target sets pc", [](TestCase &t) {
        ...
        tc.Run("VU CReg access uses CFC2/CTC2", [](TestCase &t) {
            ...
            cfc2.rs = COP2_CFC2;
            cfc2.rt = 2;
            cfc2.rd = 16; // Architectural STATUS index.
            ...
            ctc2.rs = COP2_CTC2;
            ctc2.rt = 3;
            ctc2.rd = 22; // Architectural Q index.

            std::string ctc2Code = gen.translateInstruction(ctc2);
            printGeneratedCode("VU CReg access uses CFC2/CTC2 (CTC2)", ctc2Code);
            t.IsTrue(ctc2Code.find("ctx->vu0_q") != std::string::npos, "CTC2 Q should write vu0_q");
            t.IsTrue(ctc2Code.find("GPR_U32(ctx, 3)") != std::string::npos, "CTC2 Q should read the source GPR");
