...
| Chasing data theories for an output that is intermittently ZERO/unwritten while every captured input is byte-identical between good/bad calls | "Identical inputs, different output" = a preemption yield unwound the producer before its output store — not a data/cache/logic bug (G211: 5 phases lost to data theories first) | Route to `16-runtime-concurrency-threading.md §8.1` — no-preempt diagnostic to confirm, scoped suppression to fix, then run the §8.1 wrapper audit once per port |
...
| Graphics bug (geometry/colour/texture/VU1/GS, no crash) | `15-vu1-interpreter-correctness.md` | VU1 correctness + GS state checklist + capture A/B |
