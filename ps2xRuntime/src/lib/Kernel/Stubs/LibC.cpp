...
namespace ps2_stubs
{
    ...
    // PR131: route memalign(alignment, size) to the runtime allocator so the
    // whole C allocator family shares one heap (allocator-family coherence).
    ...
    // PR131: memclr(dest, size) == memset(dest, 0, size); returns dest.
    ...
    // PS2 newlib rand()/srand(): linear congruential generator
    //   seed = seed * 0x41c64e6d + 0x3039;  return seed & 0x7FFFFFFF;
    // (RAND_MAX = 0x7FFFFFFF). DC2's iRand(n) = fptosi(n * rand() / 2^31) RELIES
    // on the full 31-bit range. The previous stub returned std::rand() & 0x7FFF
    // (0..32767), so iRand(n) = floor(n * <=32767 / 2.147e9) collapsed to 0 for
    // every map dimension. That made CAutoMapGen::CreatRoom place every room at
    // the same cell, so RandomMapMainProc's `do { CreatRoom } while(ret==0)`
    // (2nd room overlaps the 1st) never terminated -> the floor-entrance event
    // hung the dungeon loop (F60). Emulating the real LCG fixes the range and
    // makes the generator converge. Seed state is host-owned (all DC2 RNG goes
    // through these calls); newlib's default initial seed is 1.
    static uint32_t g_libcRandSeed = 1u;

    void rand(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        g_libcRandSeed = g_libcRandSeed * 0x41c64e6du + 0x3039u;
        setReturnS32(ctx, static_cast<int32_t>(g_libcRandSeed & 0x7FFFFFFFu));
    }

    void srand(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        g_libcRandSeed = getRegU32(ctx, 4);
        setReturnS32(ctx, 0);
    }
    ...
    // PR131: libgcc 64-bit signed division helper (EABI single-reg-per-64-bit).
