...
namespace ps2_stubs
{
    namespace
    {
                                         ...
                                         const MpegRegisteredCallback &callback)
        {
            ...
            uint32_t steps = 0u;
            {
                ...
                // G375 (DC2 fork): upstream additionally wraps this loop in
                // PS2Runtime::DeferredGuestYieldScope and then waits on
                // waitForGuestExecutionHandoff(); neither exists on this fork's
                // runtime yet. Dropped for the port — a reschedule raised inside a
                // movie stream callback is taken immediately instead of deferred.
        ...
        // G375 (DC2): bounded entry-point census for the FMV bring-up.
        // DC2_G375_MPEG_TRACE=1 prints the first N calls of each sceMpeg entry plus a
        // periodic aggregate, so a movie loop that never reaches the decoder is
        // distinguishable from one that reaches it and starves.
        bool g375MpegTraceEnabled()
        {
            static const bool enabled = []() {
                const char *v = std::getenv("DC2_G375_MPEG_TRACE");
                return v && v[0] != 0 && std::strcmp(v, "0") != 0;
            }();
            return enabled;
        }

        void g375MpegTrace(const char *tag, uint32_t a, uint32_t b)
        {
            if (!g375MpegTraceEnabled())
                return;
            ...
            if (n <= 8u || (n % 300u) == 0u)
            {
                std::cerr << "[G375:mpeg] " << tag << " n=" << n
                          << " a0=0x" << std::hex << a << " a1=0x" << b << std::dec << std::endl;
            }
        }


        void g375MpegTraceN(const char *tag, uint64_t a, uint64_t b, uint64_t c, uint64_t d)
        {
            if (!g375MpegTraceEnabled())
                return;
            static std::mutex traceMutex;
    ...
    {
        g375MpegTrace("AddBs", getRegU32(ctx, 4), getRegU32(ctx, 5));
        (void)runtime;
    ...
    {
        g375MpegTrace("AddStrCallback", getRegU32(ctx, 4), getRegU32(ctx, 5));
        (void)runtime;
        const uint32_t mpegAddr = getRegU32(ctx, 4);
    ...
    {
        g375MpegTrace("Create", getRegU32(ctx, 4), getRegU32(ctx, 5));
    ...
    {
        g375MpegTrace("DemuxPss", getRegU32(ctx, 4), getRegU32(ctx, 5));
        const uint32_t mpegAddr = getRegU32(ctx, 4);
        const uint32_t dataAddr = getRegU32(ctx, 5);
        const uint32_t byteCount = getRegU32(ctx, 6);
    ...
    {
        g375MpegTrace("DemuxPssRing", getRegU32(ctx, 4), getRegU32(ctx, 5));
        ...
        g375MpegTraceN("Ring[avail,consumed,decoded,cbs]", availableBytes, consumed, decodedCount, callbackEvents.size());
        dispatchStreamCallbacksUnlocked(rdram, ctx, runtime, callbackEvents);
        setReturnS32(ctx, static_cast<int32_t>(consumed));
    }
    ...
    {
        g375MpegTrace("GetPicture", getRegU32(ctx, 4), getRegU32(ctx, 5));
        const uint32_t mpegAddr = getRegU32(ctx, 4);
    ...
    {
        g375MpegTrace("Init", getRegU32(ctx, 4), getRegU32(ctx, 5));
        (void)rdram;
        (void)runtime;
    ...
    {
        g375MpegTrace("IsEnd", getRegU32(ctx, 4), getRegU32(ctx, 5));
        (void)rdram;
