...
namespace ps2_stubs
{
    ...
    {
        // newlib allocator internal. The runtime owns guest heap growth, so this
        // must stay DEAD (only newlib's own malloc body reaches it, and that body
        // is replaced by the runtime stub wrapper). G1: if it ever fires, the C
        // heap has a mixed (newlib-internal) routing path -> shout once.
        static bool warned = false;
        if (!warned)
        {
            warned = true;
            std::cerr << "[G1:WARN] malloc_extend_top called -- newlib heap-internal "
                         "path is LIVE; allocator routing is NOT coherent" << std::endl;
        }
        (void)rdram;
        (void)runtime;
        setReturnU32(ctx, 0u);
