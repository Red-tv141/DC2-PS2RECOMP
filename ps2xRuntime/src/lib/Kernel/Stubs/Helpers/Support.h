...
namespace
{
    ...
    int32_t submitDmaSend(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime, bool preferNormalCount)
    {
        ...
        PS2Memory &mem = runtime->memory();
        // [F63:dma] env-gated probe — locate the EE-thread freeze inside the
        // synchronous DMA-send path (pc frozen at sceDmaSend@0x104610).
        static const bool s_f63 = (std::getenv("DC2_TRACE_F63") != nullptr);
        static std::atomic<uint32_t> s_f63Count{0};
        const uint32_t f63ra = static_cast<uint32_t>(_mm_extract_epi32(ctx->r[31], 0));
        if (s_f63)
        {
            const uint32_t n = s_f63Count.fetch_add(1u, std::memory_order_relaxed) + 1u;
            if (n <= 64u || (n % 256u) == 0u)
                std::fprintf(stderr, "[F63:dma] n=%u ch=0x%x qwc=0x%x madr=0x%x tadr=0x%x chcr=0x%x ra=0x%x ENTER\n",
                             n, channelBase, qwc, madr, tadr, chcr, f63ra);
        }
        ...
        if (s_f63)
        {
            const uint32_t n = s_f63Count.load(std::memory_order_relaxed);
            if (n <= 64u || (n % 256u) == 0u)
