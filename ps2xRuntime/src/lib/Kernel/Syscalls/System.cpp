...
namespace ps2_syscalls
{
    ...
    // PR131: OSD Config2 (syscall 0x6E/0x6F) — date/time/language settings.
    ...
    void SetupHeap(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (runtime)
        {
            ...
            {
                std::fprintf(stderr,
                             "[F58:setupheap] a0=heapBase=0x%x a1=heapSize=0x%x -> heapLimit=0x%x "
                             "| guestHeapBase=0x%x guestHeapEnd=0x%x\n",
                             heapBase, heapSize, heapLimit, runtime->guestHeapBase(),
        ...
        if (std::getenv("DC2_TRACE_F58") != nullptr)
        {
            std::fprintf(stderr, "[F58:setupheap] no-runtime a0=heapBase=0x%x a1=heapSize=0x%x\n",
                         heapBase, heapSize);
        }
    ...
    // PR131: 0x82 InitTLB — reset the EE TLB to a sane post-init state.
    ...
    // PR131: Copy(dest, src, size) — kernel memcpy.
    ...
    // PR131: GetEntryAddress(syscallNum) — read the guest syscall table slot.
