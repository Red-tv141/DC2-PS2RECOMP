...
// F50.3: live guest worker-thread context registry (defined in ps2_runtime.cpp,
// global scope). Lets the host DC2_TRACE_HANG watchdog sample each worker's live PC.
...
namespace ps2_syscalls
{
    ...
    void StartThread(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        try
        {
            ...
            // F50.3: publish this worker's live context for the host hang watchdog.
            {
            ...
            try
            {
                ...
                while (runtime && !runtime->isStopRequested())
                {
                    ...
                    if (info->terminated.load(std::memory_order_relaxed))
                    {
                        if (s_hangTrace)
                        {
                            std::fprintf(stderr, "[F50.3:worker] tid=%d saw terminated -> exiting (pc=0x%x)\n",
                                         tid, threadCtx->pc);
                        }
            ...
            // F50.3: worker's stack-local context is about to die; remove it.
            {
                std::lock_guard<std::mutex> reg(g_guestThreadCtxMutex);
    ...
    void TerminateThread(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (s_termTrace)
        {
            std::fprintf(stderr, "[F50.3:term] tid=%d cur=%d ra=0x%x hasInfo=%d started=%d status=%d\n",
                         tid, g_currentThreadId, getRegU32(ctx, 31), (info ? 1 : 0),
                         (info ? (int)info->started : -1), (info ? info->status : -1));
        }
        if (!info)
        {
            setReturnS32(ctx, KE_UNKNOWN_THID);
        ...
        else
        {
            if (s_termTrace)
            {
                std::fprintf(stderr, "[F50.3:term] tid=%d waiting (bounded) for dormant (started=%d status=%d)\n",
                             tid, (int)info->started, info->status);
            }
            // F50.3: bounded wait for the target to unwind to dormant. On real hardware
            // TerminateThread is immediate; in this cooperative model the target only dies
            // at its next yield point, and a target starved by another guest thread (the
            // GamePad thread's infinite GamePadStep->SwitchGamePadThread loop hogs the
            // guest-execution lock) may never reach that point. terminated/forceRelease are
            // already set, so the target will die asynchronously once it is scheduled; do
            // not block the caller forever (that deadlocked InitDungeonMain -> never reached
            // LoopDungeonMain). Wait briefly for a clean dormant transition, then proceed.
            bool becameDormant = false;
            {
                std::unique_lock<std::mutex> lock(info->m);
                PS2Runtime::GuestExecutionReleaseScope releaseGuestExecution(runtime);
                becameDormant = info->cv.wait_for(lock, std::chrono::milliseconds(250), [&]()
                              { return !info->started && info->status == THS_DORMANT; });
            }
            if (s_termTrace)
            {
                std::fprintf(stderr, "[F50.3:term] tid=%d wait done dormant=%d (proceeding)\n",
                             tid, (int)becameDormant);
            }
        }
    ...
    void RotateThreadReadyQueue(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        // Cooperative yield syscall (sceRotateThreadReadyQueue / SwitchGamePadThread).
        // Release the guest-execution lock so another guest thread can run while this
        // thread yields; otherwise a guest thread spinning in a yield loop (e.g.
        // GamePadStep -> SwitchGamePadThread) holds m_guestExecutionMutex forever and
        // starves every other guest thread (F49.5/F50 deadlock at the MainLoop
        // menu->dungeon transition). The short sleep gives a waiting thread a clear
        // acquisition window and avoids a busy release->reacquire spin when no other
        // guest thread currently wants the lock.
        {
            PS2Runtime::GuestExecutionReleaseScope releaseGuestExecution(runtime);
