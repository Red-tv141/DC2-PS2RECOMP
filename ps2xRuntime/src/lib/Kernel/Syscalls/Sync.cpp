...
extern std::atomic<uint64_t> g_f60_pollSema;
extern std::atomic<uint64_t> g_f60_waitSemaBlocked;
extern std::atomic<uint32_t> g_f60_pollSema_ra;
extern std::atomic<uint32_t> g_f60_waitSema_ra;
extern std::atomic<int32_t> g_f60_waitSema_sid;
...
namespace ps2_syscalls
{
    ...
    void DeleteSema(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        // PR #136: PS2 EE BIOS DeleteSema returns the sid on success, not KE_OK.
    ...
    void SignalSema(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        // PR #136: PS2 EE BIOS SignalSema returns the sid on success (KE_SEMA_OVF overrides on overflow).
        int ret = sid;
        int beforeCount = 0;
        int afterCount = 0;
    ...
    void WaitSema(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        // PR #136: PS2 EE BIOS WaitSema returns the sid on success; error paths override below.
        int ret = sid;
        ...
        if (sema->count == 0)
        {
            ...
            sema->waiters++;
            g_f60_waitSema_ra.store(getRegU32(ctx, 31), std::memory_order_relaxed);
            g_f60_waitSema_sid.store(sid, std::memory_order_relaxed);
            g_f60_waitSemaBlocked.fetch_add(1, std::memory_order_relaxed);
            {
            ...
            sema->waiters--;
            g_f60_waitSemaBlocked.fetch_sub(1, std::memory_order_relaxed);
            if (sema->deleted)
            {
                ret = KE_WAIT_DELETE;
            }
    ...
    {
        int sid = static_cast<int>(getRegU32(ctx, 4));
        g_f60_pollSema.fetch_add(1, std::memory_order_relaxed);
        g_f60_pollSema_ra.store(getRegU32(ctx, 31), std::memory_order_relaxed);
        auto sema = lookupSemaInfo(sid);
        if (!sema)
        {
            setReturnS32(ctx, KE_UNKNOWN_SEMID);
        ...
        {
            sema->count--;
            // PR #136: PS2 EE BIOS PollSema returns the sid on success.
            setReturnS32(ctx, sid);
            return;
        }
